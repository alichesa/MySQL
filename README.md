...
- 脏读--修改了未提交，然后又发生了回滚，读取了本就不存在的数据。不可重读--数据发生了更改，导致第一次和第二次读取结果不同（数据被其他事务 ​​修改并提交​​，导致你 ​​两次读的结果不一样）。幻读--也是指读取的结果不同，但更偏向于数据的增加或者删除（不是同一行数据被修改，而是 ​​“幻影行”出现了或消失了​​，比如你查“年龄 > 18 的用户”，第一次查出来 10 条，别人插入了几条符合条件的，你再查变成 12 条）。丢弃更改--两人同时对同一个数据进行更改，导致其中一个人的更改丢弃
  
- RR（可重复读）可以防止不可重复读，但是幻读依然存在。RC（读已提交）能防止脏读，RR和RC都是利用了锁实现的。RC主要使用共享锁（S锁）和排他锁（X锁），允许读取但阻止写入，或完全阻止其他事务访问。RR利用了行级锁和间隙锁，幻读其实就是被行级锁解决了
...
- redo log记录提交后的修改，确保持久性；undo log记录修改前的数据，确保回滚时可以恢复； 原子性是通过undo log 实现的。 持久性是通过redo log实现的，redo log会写入磁盘
...
- 悲观锁是真锁，乐观锁适用于低并发、数据冲突较少的场景，通过版本比对减少锁的使用，提高并发性能，但需要处理版本冲突
  
- 视图是虚拟的表，视图本身就是一个存储的查询，视图可以将复杂的查询简化
  
- 索引查询和非索引查询本质就是在于是否是主键索引，如果是主键索引，那么就可以查询出所有的，但是如果不是主键那就回查询到主键再回表；SELECT * FROM employees WHERE employee_id = 123（主键） SELECT * FROM employees WHERE last_name = 'Smith'（非主键）
...
- 想要实现行锁，则必须加上索引，否则就是表锁； 这是为查询加索引  CREATE INDEX idx_dept ON employees(department);
...
- 在 MySQL 中，死锁通常发生在 InnoDB 存储引擎使用 行级锁 或 表级锁 时，特别是在事务并发执行时。如果两个事务需要相同的资源，但持有了彼此所需的资源，就可能发生死锁； 非常经典的情况就是交叉持有锁然后请问访问对方。
